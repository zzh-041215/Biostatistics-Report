...
% -------------------------------------------------------------------
\begin{frame}{Task 3: Type-I Error \& ROC Analysis}{Power–Type-I trade-off across kernel families}
\vspace{0.04cm}
\begin{columns}[T,totalwidth=\linewidth]
\begin{column}{0.52\linewidth}
  ...
  \vspace{0.12cm}
  {\smallnote{$d=30$, $n=100$, $B=500$, $N=200$ trials per cell. $\alpha=0.05$.}}
\end{column}
\begin{column}{0.44\linewidth}
  \vspace{-0.1cm}
  {\large\bfseries Type-I Error @ $\alpha=0.05$}\\[0.18em]
  \small
  \begin{tabular}{lccc}
  \toprule
  Family & $r=3$ & $r=5$ & $r=10$ \\
  ...
  {\large\bfseries FPR \& TPR (ROC, $k=1.15$)}\\[0.18em]
  \small
  \begin{tabular}{lcc}
  \toprule
  Family & FPR & TPR \\
  ...
  \begin{itemize}
    \item $r=3,5$: well-calibrated; $r=10$ GEXP inflates (0.12) — too many kernels degrade $\hat{\Sigma}$
    \item LAP maintains calibration at all $r$; all families' ROC well above diagonal → valid test
  \end{itemize}
\end{column}
\end{columns}
\vspace{-0.08cm}
\begin{center}
\smallnote{Warning system flags Type-I outside $[0.02, 0.08]$. Single bootstrap run → all $\alpha$ thresholds via vectorised scan.}
\end{center}
\end{frame}
